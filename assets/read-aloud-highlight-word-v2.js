(function () {
  ...
  function idsForFile(file, map) {
    ...
    if (source) {
      ...
      var ids = Array.from(document.querySelectorAll("[data-rebuild-legacy-ids] [data-id]")).filter(function (element) {
        var id = element.getAttribute("data-id");
        var text = normalise(element.textContent);
        return id && text && timestampsFor(id).length && sourceText.includes(text);
      }).map(function (element) { return element.getAttribute("data-id"); });
  ...
  function activateFile(file, startNow) {
    return Promise.all([audioByFile, Promise.resolve(timecodes)]).then(function (values) {
      timecodes = values[1] || {};
      var ids = idsForFile(file, values[0] || {});
      if (!ids.length) return;
      currentId = ids.join(",");
      currentStamps = combinedStamps(ids);
      var candidates = [];
      ids.forEach(function (id) {
  ...
  function update(media) {
    ...
    if (time > Number(stamps[stamps.length - 1].end) + 0.12 && trackIndex < trackFiles.length - 1) {
  ...
  function beginFromUrl(url) {
    ...
    if (trackFiles.length && !hasPlaybackStarted) return;
